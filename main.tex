...
\usetikzlibrary{fit,shapes,backgrounds,hobby,arrows.meta,decorations,decorations.pathreplacing}
...
\section{Введение}
\begin{frame}
Вводим героя и его задачу найти путь
\end{frame}

\begin{frame}
\frametitle{Абстракция}
Добавляем картинки возможных пожеланий в пути, и отказывается от них.
\end{frame}
...
\begin{block}{Определения}
	\begin{itemize}
		...
		\item $w(e)$ -- весовая функция $w(e)\colon E\rightarrow \mathbb{R}$, где $e=(u,v)\in E$ и $u,v\in V$;
...
\begin{frame}
	...
	\begin{tabular}{p{.49\textwidth}p{.49\textwidth}}
		...
		Последовательно перебирает все окрестные вершины, пока не выберет  целевую вершинув качестве активной. & Последовательно релаксирует по всем ребрам для каждой вершины, в итоге получает матрицу с кратчайшеми весами от $s$ до всех остальных вершин.\\
		\begin{itemize}
	...
	Иллюстративный пример с графами!

\end{frame}

\begin{frame}
	\frametitle{Как можно сделать это быстрее?}
	Джон стоит и вопрашает, на кой черт ему все эти графы, когда от стоит вот тут и видит цель своего путешествия, зачем мне перебирать всё и сразу.

\end{frame}

\begin{frame}
	\frametitle{Как можно сделать это быстрее?}
	Введем некотороую функцию $\pi(v)$, которая оценивая оставшийся путь от $v$ до $d$. Функция $\pi(v)$ называется потенциальной если удовлетворяет двум свойствам.
	...
	Пример с картой, линейкой и Джоном
\end{frame}

\begin{frame}
	\frametitle{Ещё быстрее?!}
	Если мы используем вооброжаемого близнеца Джона по имени Ножд, то он поможет сделать путь быстрее.

	Дурацкий пример с ними.
\end{frame}
...
\begin{frame}
	...
	\begin{tikzpicture}
		...
		\node[below=of land] {Картинка};
		\node[below=of hier] {Картинка};
		\node[below=of mark] {Картинка};
	\end{tikzpicture}
\end{frame}

\begin{frame}
	\frametitle{Ещё быстрее?!}
	Вставить картинки из Гольдберга
\end{frame}
...
\begin{columns}
	\column{.5\textwidth}
	Джон едет на (Герое асфальта)
	\column{.5\textwidth}
	Джон стоит в пробке в городе
\end{columns}
\end{frame}
...
\begin{frame}
	...
	\begin{block}{Определения}
		\begin{itemize}
			\item Величина $t>0$, измеряется в условных единичах времени и принимает значения из некоторого конечного множества $T$;
...
\begin{frame}
	...
	Алгоритм Дейкстры и ALT.
\end{frame}

\begin{frame}
	\frametitle{Обстановка наколяется}
	Картинки с Джоном, ядерным взрывом и нарушителями движения.
\end{frame}


\begin{frame}
	\frametitle{Быстрого решения нет}
	Картинки с Джоном скелетом на фоне пустошь
\end{frame}
...
\begin{frame}
	...
	На сцену выходит Макс рядом с интерцептором.
\end{frame}
...
\begin{frame}
	...
	Разобравшись с делами Макс решает пересечь великую пустошь, однако старые карты теперь не работают.
...
\begin{frame}
	\frametitle{Вперед в пустошь}
	\begin{block}{Задача канадского путешественника (упрощенная)}
		\begin{tabular}{>{\bfseries}rp{.75\textwidth}}
			Дано: & граф-карта $G=(V,E)$, реальный граф $H=(V,E')$, где $E'\subseteq E$, стартовая и целевая вершины $s$ и $t$.\\
	...
	\end{block}
	\pause
	Задача относится к классу $PSPACE$ и при этом $\sharp P$-сложной.
\end{frame}

\section{Киберпространство}
\begin{frame}
	\frametitle{Киберпространство}
	Кейс выходит на сцену и получает заказ от искуственного интеллекта.
	Рассмотрим задачу RCSP, но с точки зрения QoL.
\end{frame}
...
\begin{block}{Случай двух ограничений}
	Алгоритм \texttt{P+RT} основан на построении <<мостов>> в процессе работы, которые создают допустимые подпути. После на основе данных мостов восстанавливает топологию исходного графа и находит апроксимацию допустимого пути.
...
\begin{block}{Случай произвольного числа ограничений}
	Алгоритм $(1+\varepsilon)(l-1)$-\texttt{Approx} основан на построении вспомогательного графа, согласно вычисленным верхним и нижним оценкам допустимого решения, в котором выполняется поиск приближенного решения.
...
\begin{frame}
	...
	Приводит к коллапсу вселенной и слиянию всех возможных реальностей.
\end{frame}
...
\begin{frame}
	...
	Др. Джонс входит в очередную гробницу, которая хранит информацию о старом мире до коллапса.
\end{frame}
